...
::: callout-note
Here is a a fundamental overview of [high-throughput differential gene expression analysis (RNA-seq)](../lectures/02-rna-seq.qmd)
:::
...
1.  **`deseq2.colData <- data.frame(condition=factor(c(rep("control", 12), rep("desicated", 12))), type=factor(rep("single-read", 24)))`**
...
2.  **`rownames(deseq2.colData) <- colnames(data)`**
...
3.  **`deseq2.dds <- DESeqDataSetFromMatrix(countData = countmatrix, colData = deseq2.colData, design = ~ condition)`**
